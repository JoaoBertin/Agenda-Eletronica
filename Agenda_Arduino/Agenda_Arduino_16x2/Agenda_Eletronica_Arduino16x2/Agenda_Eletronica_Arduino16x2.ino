//----------------------------------------------------------------------------------------------
// Projeto: Agenda Eletrônica com ESP32 e DisplayLCD20x4
...
// de cima (No Display 16X2, 16 colunas ou caracteres)
...
// (No Display 16X2, 2 linhas)
  if (linha_atual>1) {
    linha_atual=0;
  }
  
// Repetição que dá 2 voltas para pegar os 2 digitos que formam 
// o código das letras
  while (contador<2) {
...
// Coloca a linhax como 5V, visto que a cada repetição, o valor 
// dela irá variar
      digitalWrite(linhax, HIGH);
// Leituras dos Botões, formando um curto circuito fechando as 
// colunas
      if (digitalRead(coluna1)==HIGH) {
// Apenas executa a gravação de caracteres caso o usuário já 
// tenha apertado C
        if (selec_funcao==1) {
